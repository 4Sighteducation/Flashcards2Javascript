...
(function() {
  ...
  if (!window.STUDYPLANNER_CONFIG) {
    console.error("StudyPlanner: Missing STUDYPLANNER_CONFIG. Please define configuration in Knack.");
    return;
  }
  ...
  const STUDYPLANNER_APP_CONFIG = window.STUDYPLANNER_CONFIG.appConfig || {
    'scene_1208': {
      'view_3008': {
        ...
        appUrl: window.STUDYPLANNER_CONFIG.appUrl || 'https://your-app-url.herokuapp.com/'
      }
    }
  };
  ...
  async function handleDataUpdateRequest(data, iframeWindow) {
    console.log("[Knack Script] Handling REQUEST_UPDATED_DATA request", data);
    ...
    // Check for record ID both in the direct data object and as a property
    if (!recordId) {
      console.error("[Knack Script] Cannot refresh data - missing record ID in request", data);
